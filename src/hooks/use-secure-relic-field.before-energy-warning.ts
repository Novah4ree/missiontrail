...
export function useSecureRelicField({
  ...
  useEffect(() => {
    ...
    if (pending?.stage === candidate) {
      ...
      else fallbackStageRef.current = { stage: candidate, count: pending.count + 1 };
    } else {
    ...
    try {
      ...
      if (__DEV__) {
        console.log('[RELIC FIELD]', {
          ...
          locked: field.zones.filter(
            (zone) => zone.availability === 'locked',
          ).length,
        });
      }
      ...
      initialFieldRetryNotBeforeRef.current =
        Date.now() + retryCooldownMs;
      ...
      if (isGpsLockError) {
        setStatus("improving_accuracy");
        setMessage(
          "Refining GPS… Hold steady for a moment.",
        );
      } else {
    ...
    try {
      ...
      if (__DEV__) {
        console.log("[RELIC RADAR]", {
          ...
          availableCount: result.signals?.filter(
            (signal) => signal.availability === 'available',
          ).length ?? 0,
          lockedCount: result.signals?.filter(
      ...
      if (
        ...
        setMessage(
          'Relic signal temporarily weak… Target remains locked.',
        );
      ...
      if (activeSignal) {
        ...
        setDistanceFeet(
          activeSignal.distanceFeet,
        );

        setBearingDegrees(
          activeSignal.bearingDegrees,
        );

        setClueStrength(
          activeSignal.clueStrength,
        );
      ...
      if (result.distanceFeet !== undefined) {
        setDistanceFeet(result.distanceFeet);
      }
      if (result.bearingDegrees !== undefined || result.encounterType === 'ambient') {
  ...
  const selectSignal = useCallback((assignmentId: string) => {
  ...
  const useAutoNearest = useCallback(() => {
    ...
    if (nearestAvailable) {
      ...
      setScanState('empty');
      setMessage('No available signals are loaded. Tap Find Hidden Relic to refresh.');
    }
  }, [signals]);
