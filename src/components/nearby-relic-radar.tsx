...
export function NearbyRelicRadar({
  ...
  const availableSignals = field.signals.filter(
    (signal) => signal.availability === 'available',
  );
  const autoActive =
    field.targetMode === 'auto' && availableSignals.length > 0;
  const autoUnavailable = field.isBusy || availableSignals.length === 0;
          ...
          <Pressable
            ...
            accessibilityHint={
              availableSignals.length === 0
                ? 'Tap Find Hidden Relic to refresh available signals'
                : 'Targets the closest available relic signal'
            }
            accessibilityState={{ selected: autoActive, disabled: autoUnavailable }}
            disabled={autoUnavailable}
            onPress={() => {
              if (__DEV__) console.log('[RELIC UI] Auto Nearest pressed');
              field.useAutoNearest();
            }}
            style={({ pressed }) => [
              styles.autoButton,
              autoActive && styles.autoButtonSelected,
              autoUnavailable && styles.autoButtonDisabled,
              pressed && styles.pressed,
            ]}
          >
            <Ionicons
              name={autoActive ? 'navigate' : 'navigate-outline'}
              size={13}
              color={autoActive ? '#160A24' : '#DDA7FF'}
            />
            <Text
              style={[
                styles.autoText,
                autoActive && styles.autoTextSelected,
              ]}
            >
              {availableSignals.length === 0
...
const styles = StyleSheet.create({
  ...
  autoButtonDisabled: { opacity: 0.58 },
