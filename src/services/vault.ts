...
// Purpose: Synchronizes server vault cache.
export async function syncServerVaultCache() {
  const { data, error } = await supabase
    .from('user_relic_collections')
    .select('id, relic_id, collected_at, xp_awarded')
    .order('collected_at', { ascending: true });

  if (error) {
    throw new Error('Vault sync is temporarily unavailable');
  }

  return cacheServerCollections(
    (data ?? []).map((item) => ({
