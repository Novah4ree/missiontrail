...
export default function HomeScreen() {
  ...
  return (
    <View style={styles.screen}>
      ...
      <View style={[styles.fixedOverlay, { paddingTop: safeArea.top, paddingBottom: safeArea.bottom }]}>
...
function watchLiveLocation(onLocationChange: (location: Location.LocationObject) => void) {
  return Location.watchPositionAsync({ accuracy: Location.Accuracy.High, distanceInterval: 6, timeInterval: 2500 }, onLocationChange);
}
...
function getLiveStats(walkedMiles: number) {
  return { distance: walkedMiles.toFixed(2), steps: Math.max(0, Math.round(walkedMiles * 2200)).toLocaleString(), items: '2/5' };
}
function getLatestGpsPoint(points: Location.LocationObject[]) { return points[points.length - 1]; }
...
function getMetersBetweenPoints(start: Location.LocationObject, end: Location.LocationObject) {
  ...
  const startLat = (start.coords.latitude * Math.PI) / 180;
  const endLat = (end.coords.latitude * Math.PI) / 180;
  const latChange = ((end.coords.latitude - start.coords.latitude) * Math.PI) / 180;
  const longChange = ((end.coords.longitude - start.coords.longitude) * Math.PI) / 180;
  const halfChordLength = Math.sin(latChange / 2) ** 2 + Math.cos(startLat) * Math.cos(endLat) * Math.sin(longChange / 2) ** 2;
...
function makeMapRegion(location: Location.LocationObject): Region {
  return { latitude: location.coords.latitude, longitude: location.coords.longitude, latitudeDelta: 0.012, longitudeDelta: 0.012 };
}
...
function renderFootprintMarkers(coordinates: ReturnType<typeof makeMapCoordinate>[]) {
  return coordinates.map((coordinate, index) => (
    <Marker key={`${coordinate.latitude}-${coordinate.longitude}-${index}`} coordinate={coordinate} anchor={{ x: 0.5, y: 0.5 }}>
...
function renderHeader() {
  return (
    <View style={styles.headerBar}>
      <View style={styles.logoOrbit}><Ionicons name="planet-outline" size={isSmallPhone ? 21 : 24} color="#ff68f4" /></View>
...
function renderTopStatsCard(liveStats: ReturnType<typeof getLiveStats>) {
  const stats = [
    { label: 'Distance', value: liveStats.distance, unit: 'mi', icon: 'location-outline' as NeonIconName, color: '#00e5ff' },
    { label: 'Steps', value: liveStats.steps, unit: 'steps', icon: 'footsteps-outline' as NeonIconName, color: '#ff63f7' },
    { label: 'Items', value: liveStats.items, unit: 'found', icon: 'gift-outline' as NeonIconName, color: '#a855f7' },
  ];
  return (
  ...
  return (
    <View style={styles.warningCard}>
      ...
      <View style={styles.warningCopy}>
        <Text style={styles.warningTitle}>{locationError ? 'Location blocked' : 'Movement too fast'}</Text>
        <Text style={styles.warningText}>{locationError ?? 'Driving is not allowed. Slow down to continue tracking.'}</Text>
      </View>
    </View>
  );
}
function renderGpsStatusBadge(isTracking: boolean, currentSpeedMph: number, safeBottom: number) {
  return (
    <View style={[styles.mapCenterBadge, { bottom: safeBottom + tabBarHeight + 28 }]} pointerEvents="none">
      ...
      <Text style={styles.mapCenterText}>{isTracking ? 'Tracking footsteps' : 'Tracking paused'} | {currentSpeedMph.toFixed(1)} MPH</Text>
    </View>
  );
}
  ...
  return (
    <View style={styles.floatingButtons}>
      {mapButtons.map((button) => (
        ...
        >
          <Ionicons name={button.icon as any} size={isSmallPhone ? 19 : 21} color="#d9f7ff" />
        </Pressable>
      ))}
    </View>
  );
}
function renderBottomTabBar(router: any) {
  return (
        ...
        return (
          <Pressable
            ...
            onPress={() => {
              ...
              if (tab.key === 'map') router.push('/home-backup');
            }}
          >
            ...
            <Text style={[styles.tabLabel, isActiveTab && styles.activeTabLabel]} numberOfLines={1}>{tab.label}</Text>
          </Pressable>
        );
      })}
    </View>
...
const styles = StyleSheet.create({
  ...
  cosmicOverlay: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(18, 10, 46, 0.18)' },
  fixedOverlay: { ...StyleSheet.absoluteFillObject, paddingHorizontal: sidePadding },
  topOverlay: { position: 'absolute', left: sidePadding, right: sidePadding, gap: 6 },
  bottomOverlay: { position: 'absolute', left: sidePadding, right: sidePadding },
  headerBar: { height: isSmallPhone ? 30 : 34, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 9 },
  logoOrbit: { width: isSmallPhone ? 25 : 28, height: isSmallPhone ? 25 : 28, borderRadius: 14, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: 'rgba(168, 85, 247, 0.6)', backgroundColor: 'rgba(86, 19, 216, 0.18)' },
  headerTitle: { color: '#ffffff', fontSize: isSmallPhone ? 14 : 16, fontWeight: '900', fontStyle: 'italic', letterSpacing: 1.2, textShadowColor: '#a855f7', textShadowRadius: 7 },
  bellWrap: { position: 'absolute', right: 4, width: 30, height: 30, alignItems: 'center', justifyContent: 'center' },
  notificationDot: { position: 'absolute', right: 5, top: 4, width: 16, height: 16, borderRadius: 8, alignItems: 'center', justifyContent: 'center', backgroundColor: '#ff2d75' },
  notificationText: { color: '#ffffff', fontSize: 10, fontWeight: '900' },
  statsCard: { minHeight: isSmallPhone ? 50 : 54, borderRadius: 14, borderWidth: 1, borderColor: '#a855f7', backgroundColor: 'rgba(8, 5, 28, 0.88)', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 8, elevation: 8 },
  statItem: { flex: 1, alignItems: 'center', gap: 1, paddingHorizontal: 2, borderRightWidth: 1, borderRightColor: 'rgba(168, 85, 247, 0.18)' },
  statLabelRow: { flexDirection: 'row', alignItems: 'center', gap: 2 },
  statLabel: { color: '#ffffff', fontSize: isSmallPhone ? 7 : 8, fontWeight: '900', textTransform: 'uppercase' },
  statValue: { color: '#ffffff', fontSize: isSmallPhone ? 14 : 16, fontWeight: '900' },
  statUnit: { color: '#d9ddff', fontSize: isSmallPhone ? 7 : 8, fontWeight: '700' },
  warningCard: { minHeight: 48, borderRadius: 14, borderWidth: 1, borderColor: '#ff2d75', backgroundColor: 'rgba(65, 6, 26, 0.9)', flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 10 },
  warningCopy: { flex: 1, gap: 2 },
  warningTitle: { color: '#ffffff', fontSize: 13, fontWeight: '900' },
  warningText: { color: '#ffffff', fontSize: 10, fontWeight: '600' },
  mapCenterBadge: { position: 'absolute', alignSelf: 'center', borderRadius: 999, borderWidth: 1, borderColor: 'rgba(0, 229, 255, 0.5)', backgroundColor: 'rgba(3, 2, 18, 0.82)', paddingHorizontal: 12, paddingVertical: 6, alignItems: 'center' },
  mapCenterTitle: { color: '#ffffff', fontSize: 10, fontWeight: '900', letterSpacing: 1 },
  mapCenterText: { color: '#74eaff', fontSize: 9, fontWeight: '700' },
  floatingButtons: { position: 'absolute', right: sidePadding, top: '39%', gap: 15 },
  floatingButton: { width: isSmallPhone ? 40 : 44, height: isSmallPhone ? 40 : 44, borderRadius: 22, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: '#d946ef', backgroundColor: 'rgba(15, 7, 39, 0.92)', elevation: 8 },
  pressed: { opacity: 0.72, transform: [{ scale: 0.97 }] },
  mapFootprintGlow: { width: 28, height: 28, borderRadius: 14, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(255, 99, 247, 0.06)' },
  mapFootprintImage: { width: 64, height: 64 },
  currentFootprintGlow: { width: 44, height: 44, borderRadius: 32, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(255, 99, 247, 0.05)' },
  currentFootprintImage: { width: 64, height: 64 },
  tabBar: { minHeight: tabBarHeight, maxHeight: tabBarHeight, borderRadius: 14, borderWidth: 1, borderColor: '#6d28d9', backgroundColor: 'rgba(6, 4, 26, 0.95)', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-around', paddingHorizontal: 4, paddingVertical: 4, elevation: 9 },
  tabButton: { flex: 1, height: tabBarHeight - 6, alignItems: 'center', justifyContent: 'center', gap: 4 },
  tabIconWrap: { width: isSmallPhone ? 38 : 44, height: isSmallPhone ? 38 : 44, borderRadius: 22, alignItems: 'center', justifyContent: 'center' },
  activeTabIconWrap: { borderWidth: 1, borderColor: '#00e5ff', backgroundColor: 'rgba(86, 19, 216, 0.32)' },
  tabIcon: { width: isSmallPhone ? 61 : 56, height: isSmallPhone ? 61 : 56 },
  tabLabel: { color: '#ffffff', fontSize: isSmallPhone ? 9 : 10, fontWeight: '800' },
  activeTabLabel: { color: '#00e5ff' },
});