...
set search_path = ''
as $$
  -- Exact points remain service-role-only. The Edge Function converts them to
  -- distance/bearing summaries and never returns these coordinate columns.
  select
    assignment.id,
    assignment.status,
  ...
  where assignment.user_id = p_user_id
    ...
    and assignment.eligibility_status in ('eligible', 'overridden', 'locked')
...
comment on function public.server_list_nearby_relic_contexts(uuid) is
  'Service-only active radar contexts, including opaque locked assignments. Exact points must never be exposed to Expo.';
